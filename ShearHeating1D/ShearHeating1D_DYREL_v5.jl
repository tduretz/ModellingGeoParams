...
function ResidualMechanics_τII!(RV, Vx, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, τii, ε̇xy, ε̇ii, Tv, rhs )
    rel = 0.1
    @. ε̇xy         = 0.5*(Vx[2:end] - Vx[1:end-1])/Δy
    @. ε̇ii         = sqrt(ε̇xy^2)
    Viscosity_τii!(η_phys, τii, Tv, flow_nd)
    @. η           = (1.0-rel)*η + rel*(1.0/η_phys + 1.0/(ηe))^(-1)
    @. τxy         =  2 * η * (ε̇xy + τxy0/(2*ηe))
    @. τii         = sqrt.(τxy.^2)
    @. RV[2:end-1] =  (τxy[2:end]  - τxy[1:end-1]) / Δy 
end
...
function main()
    ...
    ε0         = nondimensionalize(5e-13s^-1, CharDim)
    ...
    Viscosity_ε̇ii!(η, ε̇ii, Tv, flow_nd)
    τii = 2.0 .* η .* ε̇ii
    ...
    for it=1:Nt
        ...
        @views for iter=1:niter
            ...
            # Residuals
            if style==0 ResidualMechanics_εII!(RV, Vx, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, ε̇xy, ε̇ii, Tv, 1.0 )      end
            if style==1 ResidualMechanics_τII!(RV, Vx, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, τii, ε̇xy, ε̇ii, Tv, 1.0 ) end
            ...
            if mod(iter, nout) == 0 
                ...
                if style==0 ResidualMechanics_εII!(KδV1,  Vx, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, ε̇xy, ε̇ii, Tv, 0.0 )     end
                if style==0 ResidualMechanics_εII!(KδV, Vxit, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, ε̇xy, ε̇ii, Tv, 0.0 )     end
                # τii1 = τii 
                # η1   = η 
                if style==1 ResidualMechanics_τII!(KδV1, Vx, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, τii, ε̇xy, ε̇ii, Tv, 1.0 ) end
                # τii  = τii1
                # η   = η1 
                if style==1 ResidualMechanics_τII!(KδV, Vxit, flow_nd, η, ηe, η_phys, Δy, τxy, τxy0, τii, ε̇xy, ε̇ii, Tv, 1.0 ) end
