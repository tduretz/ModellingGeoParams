...
@views function Residuals!(Rx, Ry, Vx, Vy, Pt, P_it, ε̇, τ, τ0, rheo, ηvec, ηvev, Dx, Dy, Δt, Δx, Δy )

    ε̇.∇V   .= diff(Vx[:,2:end-1], dims=1)./Δx .+ diff(Vy[2:end-1,:], dims=2)./Δy
    Pt     .=  P_it .- rheo.ξc .*ε̇.∇V
    # strain rates
...
@views function MainMechanicsDiagPC2D(n)
    ...
    r_inc = [sqrt(0.01) 0.09 0.05 0.08 0.08  0.1 0.07 0.08 0.07 0.07] .*1.8
    ...
    if PC == :diag
        if ismaxloc
            DiagMechanics2Dx!( Dx, ηc_maxloc, 0.0.*rheo.ξc, ηv_maxloc, Δx, Δy, bx, Ncx, Ncy )
            DiagMechanics2Dy!( Dy, ηc_maxloc, 0.0.*rheo.ξc, ηv_maxloc, Δx, Δy, by, Ncx, Ncy )
        else
            DiagMechanics2Dx!( Dx, rheo.ηeffc, 0.0.*rheo.ξc, rheo.ηeffv, Δx, Δy, bx, Ncx, Ncy )
            DiagMechanics2Dy!( Dy, rheo.ηeffc, 0.0.*rheo.ξc, rheo.ηeffv, Δx, Δy, by, Ncx, Ncy )
        end
    end
    for it=1:Nt
        ...
        hVx[:,2:end-1] .= 2.0./(λmaxlocVx)*CFL*1.3
        hVy[2:end-1,:] .= 2.0./(λmaxlocVy)*CFL*1.3
        ...
        maxloc!(ηc_maxloc, ηvec)
        
        rheo.ξc .= ηvec / min(Lx,Ly) * min(Δx, Δy)*20
        # rheo.ξc .= ηvec / sqrt(min(Lx,Ly)) * sqrt(min(Δx, Δy))

        GershgorinMechanics2Dx_Local!( λmaxlocVx, rheo.ηeffc, 0.0*rheo.ξc, rheo.ηeffv, Dx, Δx, Δy, Ncx, Ncy )
        λmaxlocVx .= maximum(λmaxlocVx)

        GershgorinMechanics2Dy_Local!( λmaxlocVy, rheo.ηeffc, 0.0*rheo.ξc, rheo.ηeffv, Dy, Δx, Δy, Ncx, Ncy )
        λmaxlocVy .= maximum(λmaxlocVy)
        ...
        @time @views for iter=1:niter
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                k = copy(rheo.ξc)
                rheo.ξc .= 0
                Residuals!(KδVx1, KδVy1, Vx,      Vy,      Pt, P_it, ε̇, τ, τ0, rheo, ηvec, ηvev, Dx, Dy, Δt, Δx, Δy )
                Residuals!(KδVx,  KδVy,  Vx.-δVx, Vy.-δVy, Pt, P_it, ε̇, τ, τ0, rheo, ηvec, ηvev, Dx, Dy, Δt, Δx, Δy )
                rheo.ξc .= k
                ...
                # rheo.ξc .= 2*ηvec / min(Lx,Ly) * max(Δx, Δy) / c
                ...
                @show (c, λmin, maximum(λmaxlocVx), log10(maximum(λmaxlocVx)) - log10(λmin))
                # @show (λmin, maximum(λmaxlocVy), log10(maximum(λmaxlocVx)) - log10(λmin))
                # @show (minimum(rheo.λ̇itc), maximum(rheo.λ̇itc))
                # if ( isnan(errVx) ) error() end
                # ( errVx < ϵ ) && break
        ...
        @show sum(ηvec.==minimum(ηvec))/Ncx/Ncy
        @show sum(ηvec.< ηvec[1])/Ncx/Ncy
