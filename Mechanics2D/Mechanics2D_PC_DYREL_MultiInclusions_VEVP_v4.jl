...
@views function VEP(ϵ̇xx, ϵ̇yy, ϵ̇xy, ϵ̇zz, ∇V, params, ηv, ηe, ξ, λ̇it, Fc)
    ...
    if F>0
        ...
        τzz           = -(τxx + τyy)
        τII           = sqrt( 0.5*(τxx.^2 + τyy^2 + τzz^2) + τxy^2 )
        F             = τII - c*cos(ϕ) - p*sin(ϕ) - ηvp*λ̇it
...
@views function MainMechanicsDiagPC2D()
    ...
    yield      = (
        ...
        ηvp    = nondimensionalize(0.25Pas, CharDim),
        rel    = 1.0,
        Fiter  = true
    )
    ...
    ηi    = (s=η_mat*1e-1, w=η_mat*1e-1) 
    Gi    = (s=G_mat*1e-1, w=G_mat*1e-1) 
