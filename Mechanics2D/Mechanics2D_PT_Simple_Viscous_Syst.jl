using GeoParams, Plots, Printf, MathTeXEngine, BenchmarkTools, Statistics, CSV, DataFrames
...
@views function Residuals!(Rx, Ry, ε̇, τ, τ0, Vx, Vy, Pt, ηvec, ηvev, rheo, Δt, Δx, Δy)
    # divergence - pressure
...
@views function MainMechanicsDiagPC2D(n, dtP_fact, dtV_fact, θ_fact)
    ...
    θx          = zeros(Ncx+1, Ncy+2)
    a1x         = zeros(Ncx+1, Ncy+2)
    a2x         = zeros(Ncx+1, Ncy+2)
    θy          = zeros(Ncx+2, Ncy+1)
    a1y         = zeros(Ncx+2, Ncy+1)
    a2y         = zeros(Ncx+2, Ncy+1)
    ...
    for it=1:Nt
        ...
        dtPt .= dtP_fact * 2.0*ηvec ./ max(Ncx, Ncy)

        iters = 0
        @time @views for iter=1:niter
            iters += 1

            Residuals!(Rx, Ry, ε̇, τ, τ0, Vx, Vy, Pt, ηvec, ηvev, rheo, Δt, Δx, Δy)
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                ( errVx < ϵ && errPt < ϵ || fail ) && break
            end
            if ( isnan(errVx) ) break end

            θx  .= sqrt.(4.0*λmin.*hVx)*θ_fact
            a1x .= 1.0.-θx # Vdmp/Ncx
            a2x .= 1.0
            @. ∂Vx∂τ                 = a1x*∂Vx∂τ + a2x.*Rx
            @. δVx[2:end-1,2:end-1]  = dtV_fact*hVx[2:end-1,2:end-1]*∂Vx∂τ[2:end-1,2:end-1] 
            @. Vx[2:end-1,2:end-1]  += δVx[2:end-1,2:end-1]

            # a1y = (2 .- c.*hVy) ./ (2 .+ c.*hVy)
            # a2y = 2*hVy ./(2 .+ c*hVy)
            θy  .= sqrt.(4.0*λmin.*hVy)*θ_fact
            a1y .= 1.0.-θy # Vdmp/Ncx
            a2y .= 1.0 # c --> 2 ./hVy .* (hVy-1)
            @. ∂Vy∂τ                 = a1y*∂Vy∂τ + a2y.*Ry
            @. δVy[2:end-1,2:end-1]  = dtV_fact*hVy[2:end-1,2:end-1]*∂Vy∂τ[2:end-1,2:end-1]
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                Residuals!(KδVx1, KδVy1, ε̇, τ, τ0, Vx, Vy, Pt, ηvec, ηvev, rheo, Δt, Δx, Δy)
                Residuals!(KδVx,   KδVy, ε̇, τ, τ0, Vx.-δVx, Vy.-δVy, Pt, ηvec, ηvev, rheo, Δt, Δx, Δy)
    ...
    itertot
    return (itertot, dtPt[1], hVx[2,2], θx[2,2], λmin, λmaxlocVx[2,2], λminPt, Δx, ηvec[1])
end

function SystematicsAll()

    test_range = LinRange(0.6, 1.80, 16)
    iters      = zero(test_range)
    data       = zeros(length(test_range)^3, 12)

    it = 0
    for it1 in eachindex(test_range), it2 in eachindex(test_range), it3 in eachindex(test_range)

        it += 1
        a = MainMechanicsDiagPC2D(2, test_range[it1], test_range[it2], test_range[it3])
        data[it,1]      = test_range[it1]
        data[it,2]      = test_range[it2]
        data[it,3]      = test_range[it3]
        data[it,4:end] .= a[1:end]
    end
    # show(stdout, "text/plain", data)
    df = DataFrame(data, [:dtP_fact, :dtV_fact, :θ_fact, :iters, :dtP, :dtV, :theta, :λminVx, :λmaxVx, :λminPt, :Δx, :η ])
    show(stdout, "text/plain", df)
    CSV.write( "df2_bis.csv", df)
end



# MainMechanicsDiagPC2D(4, 1.8)
MainMechanicsDiagPC2D(1, 0.87, 1.0, 2.4) # 600
# MainMechanicsDiagPC2D(1, 1.72, 1.32, 1.8) # 600

# MainMechanicsDiagPC2D(2, 0.87*2.0, 1.0, 2.4) # 1500

# MainMechanicsDiagPC2D(4, 0.87*2.7, 1.0, 2.4)
