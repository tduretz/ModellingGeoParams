...
function DiagMechanics2Dx!( Dx, ηc, kc, ηv, Δx, Δy, b, Ncx, Ncy )
    ...
    @. Dx[:,2:end-1] = 4//3*ηE/Δx^2 + 4//3*ηW/Δx^2 + ηS/Δy^2 + ηN/Δy^2 + 2/Δx# + kW/Δx^2 + kE/Δx^2
end
...
function DiagMechanics2Dy!( Dy, ηc, kc, ηv, Δx, Δy, b, Ncx, Ncy )
    ...
    @. Dy[2:end-1,:] = 4//3*ηS/Δy^2 + 4//3*ηN/Δy^2 + ηW/Δx^2 + ηE/Δx^2 + 2/Δy# + kS/Δy^2 + kN/Δy^2
end
...
@views function MainMechanicsDiagPC2D(n)
    ...
    G_mat      = nondimensionalize(1000Pa,  CharDim)
    plastic    = (
    ...
    rheo       = (
        ...
        ξc         = ones(Ncx+0, Ncy+0)*1,
        ξv         = ones(Ncx+1, Ncy+1)*1,
    ...
    Dx         = ones(Ncx+1, Ncy+2)
    Dy         = ones(Ncx+2, Ncy+1)
    cVx        = ones(Ncx+1, Ncy+2)
    cVy        = ones(Ncx+2, Ncy+1)
    ...
    # Buoyancy
    x0, y0 = 0.5, 0.5
    # by[(xc.-x0).^2 .+ (yv'.-y0).^2 .< 0.1^2 ] .= 1.0

    # Multiple circles with various viscosities
    ηi    = (s=η_mat*1e-3, w=η_mat*1e-3) 
    Gi    = (s=G_mat*5e10, w=G_mat*5e10)
    Gi    = (s=G_mat*1.0, w=G_mat*1.0) 
    sx = -0.1
    sy = -0.1
    ...
    for it=1:Nt
        ...
        rheo.ξc .= rheo.ηeffc*200
        λmaxlocPt .=  rheo.ξc.*(1.0./rheo.ξc*2000 .+ 2.0/Δx .+ 2.0/Δy)        
        hPt .= 2.0./sqrt.(λmaxlocPt)*1.0 

        # cPt = 47.5
        # cPt .= sqrt.(λmaxlocPt).*(0.5./rheo.ηeffc)*1.6

        # sx = -0.4
        # sy = -0.1
        # cPt .= sqrt.(λmaxlocPt).*(0.5./rheo.ηeffc)*0.29 # 9600 

        # sx = -0.1
        # sy = -0.1
        # cPt .= sqrt.(λmaxlocPt).*(0.5./rheo.ηeffc)*0.8 # 5400
        ...
        @time @views for iter=1:niter
            ...
            ResidualContinuity!(Rp, Vx,      Vy,      Pt, bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, comp)
            ...
            @. ∂Pt∂τ                 = (2-cPt*hPt)/(2+cPt*hPt)*∂Pt∂τ + 2*hPt/(2+cPt*hPt).*Rp #.* rheo.ηeffc
            @. δPt                   = hPt*∂Pt∂τ
            @. Pt                   += δPt
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                # cVx[:,2:end-1] .= 2.0*sqrt.(λmin.*λmaxlocVx./(λmin .+ λmaxlocVx))*cfact
                # cVy[2:end-1,:] .= 2.0*sqrt.(λmin.*λmaxlocVy./(λmin .+ λmaxlocVy))*cfact
                cVx .= 2.0*sqrt(λmin)*cfact
                cVy .= 2.0*sqrt(λmin)*cfact
                @show (λmin)

                # ResidualContinuity!(KδPt1, Vx,      Vy,      Pt,      bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, comp)
                # ResidualContinuity!(KδPt,  Vx.-δVx, Vy.-δVy, Pt, bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, comp)
                # λminPt = abs(sum(.-δPt.*(KδPt1.-KδPt))/(sum(δPt.*δPt) ) )
                # @show (λminPt)

                ResidualContinuity!(KδPt1, Vx,      Vy,      Pt,      bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, comp)
                ResidualContinuity!(KδPt,  Vx,      Vy,      Pt.-δPt, bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, comp)
                λminPt = abs(sum(.-δPt .* (KδPt1.-KδPt))/(sum(δPt.*δPt) ) )
                @show (λminPt)
                @show hPt[1]
                @show minimum(cPt), maximum(cPt)
                @show cPt[1] / λminPt
                @show cPt[1] / sqrt(λminPt)
                @show cPt[1] / (λminPt)^2
                # @show 2/sqrt(λminPt)
                # cPt = 2.0*λminPt
                @show 2.0*sqrt(λminPt)*cfact
                @show 2.0*sqrt(λmin)*cfact
                @show 2.0*sqrt(λmaxlocPt[1])*cfact
                @show 2.0*sqrt(λmaxlocVx[1])*cfact
                # @show hPt/cPt

                cPt .=  2.0*sqrt(λminPt)*cfact*4.0 #.- 3#- 2.0*sqrt.(λminPt)*cfact
                ...
                printstyled("$(@sprintf("Iteration %05d --- Time step %4d \n", iter, it ))", color=:blue)
        ...
        if mod(it, 1)==0 || it==1
            ...
            p3=heatmap(ustrip.(dimensionalize(xc[2:end-1], m, CharDim)./1e3), ustrip.(dimensionalize(yc[2:end-1], m, CharDim)./1e3), ustrip.(dimensionalize(Pt, Pa, CharDim))', title = "$(mean(probes.iters[1:it])) iterations", aspect_ratio=1.0, color=:turbo )
            ...
            @show minimum(rheo.ηeffv), maximum(rheo.ηeffv)
        end
    end
