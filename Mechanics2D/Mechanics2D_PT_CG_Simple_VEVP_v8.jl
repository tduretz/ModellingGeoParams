...
# Poisson just for a simple test
function Poisson2D!(f1, u1, qx, qy, b, ηx, ηy, Δx, Δy )
    u1[1,:]   = -u1[2,:]
    u1[end,:] = -u1[end-1,:]
    u1[:,1]   = -u1[:,2]
    u1[:,end] = -u1[:,end-1]
    qx .= - ηx.*diff(u1[:,2:end-1], dims=1)/Δx
    qy .= - ηy.*diff(u1[2:end-1,:], dims=2)/Δy
    f1[2:end-1,2:end-1] .= b .- diff(qx,dims=1)/Δx .- diff(qy,dims=2)/Δy
end
...
# qx = zeros(Ncx+1,Ncy)
# qy = zeros(Ncx,Ncy+1)
# ηx = 0.5*(rheo.ηv[:,1:end-1] .+ rheo.ηv[:,2:end-0])
# ηy = 0.5*(rheo.ηv[1:end-1,:] .+ rheo.ηv[2:end-0,:])
# b         = ones(Ncx,Ncy)
# b        .= rheo.ηc 
# Poisson2D!(r, x, qx, qy, b, ηx, ηy, Δx, Δy )
# p  = zero(r)
# Ap = zero(r)
# p .= r
# for itCJ = 1:500
#     dotr0= dot(r,r)
#     Poisson2D!(Ap, p, qx, qy, 0.0.*b, ηx, ηy, Δx, Δy )
#     α = dotr0/dot(p, Ap)
#     x .+= α.*p 
#     r .-= α.*Ap
#     @show itCJ, norm(r)/length(r)
#     if norm(r)/length(r)<1e-9 break end
    # β = dot(r,r)/dotr0
    # p .= r .+ β.*p
# end
    ...
    for it=1:1#Nt
        ...
        for it1=1:10
            ...
            if inner_solver==:CG
                ...
                max_its = length(Rx)
                tot_its = max_its
                for its = 1:5000#max_its
                    dot0 = sum(Rx.*Rx) + sum(Ry.*Ry)
                    Residuals_SchurComplement!(Apx, Apy, px, py, bx_SC, by_SC,     ε̇, τ, τ0, rheo, rheo.ηeffc, rheo.ηeffv, Dx, Dy, Δt, Δx, Δy )
                    α      = dot0/(sum(px.*Apx) + sum(py.*Apy))               
                    Vx   .-= α .* px 
                    Vy   .-= α .* py
                    Rx   .-= α .* Apx
                    Ry   .-= α .* Apy
                    dot1 = sum(Rx.*Rx) + sum(Ry.*Ry)
                    β    = dot1/dot0
                    px  .= Rx .+ β .* px
                    py  .= Ry .+ β .* py
                    if sqrt(dot1)/(length(Rx)+length(Ry)) < ϵrel/1000
                        @printf("CG: It. %04d: res. = %2.6e\n", its, sqrt(dot1)/(length(Rx)+length(Ry))) 
                        tot_its = its
                        break
                    end
                end

            elseif inner_solver==:PT
                iter_count = 0.
                @time @views for iter=1:niter
                    ...
                    if mod(iter, nout) == 0 || iter==1
                        ...
                        # @printf("Rx = %2.4e --- Ry = %2.4e --- Rp = %2.4e --- max(Fc) = %lf --- max(Fv) = %lf  \n", errVx_in/errVx0_in, errVy_in/errVy0_in, errPt,maximum(rheo.Fc), maximum(rheo.Fv))
