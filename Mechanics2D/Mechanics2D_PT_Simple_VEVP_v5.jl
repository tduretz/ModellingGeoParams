...
@views function MainMechanicsDiagPC2D(n)
    ...
    for it=1:Nt
        ...
        # New dtp after discussion with Yury on 19/02/24
        # 0.028
        # 0.022
        dtPt .= 0.022 * ηvec[1] * ((0.5*(hVx[1:end-1,2:end-1] .+ hVx[2:end,2:end-1]) + 0.5*(hVy[2:end-1,1:end-1] .+ hVy[2:end-1,2:end-0])) ./ max(Δx,Δy)^2).^(-1)
