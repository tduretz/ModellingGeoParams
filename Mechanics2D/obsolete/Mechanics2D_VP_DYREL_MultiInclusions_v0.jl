...
function DiagMechanics2Dx!( Dx, ηc, kc, ηv, Δx, Δy, b, Ncx, Ncy )
    ...
    # @. Dx[:,2:end-1] = 2*4//3*ηE/Δx^2 + 2*4//3*ηW/Δx^2 + 2*ηS/Δy^2 + 2*ηN/Δy^2 + 2*kW/Δx^2 + 2*kE/Δx^2
    # @. Dx[:,2:end-1] *= 200
end
...
function DiagMechanics2Dy!( Dy, ηc, kc, ηv, Δx, Δy, b, Ncx, Ncy )
    ...
    # @. Dy[2:end-1,:] = 2*4//3*ηS/Δy^2 + 2*4//3*ηN/Δy^2 + 2*ηW/Δx^2 + 2*ηE/Δx^2 + 2*kS/Δy^2 + 2*kN/Δy^2
    # @. Dy[2:end-1,:] *= 200
end
...
function GershgorinMechanics2Dx_Local!( λmaxloc, ηc, kc, ηv, Dx, Δx, Δy, Ncx, Ncy )
    ηW = zeros(Ncx+1, Ncy); ηW[2:end-0,:] .= ηc; ηW[1,:]   = ηW[2,:]
    ηE = zeros(Ncx+1, Ncy); ηE[1:end-1,:] .= ηc; ηE[end,:] = ηE[end-1,:]
    kW = zeros(Ncx+1, Ncy); kW[2:end-0,:] .= kc*0.0; kW[1,:]   = kW[2,:]
    kE = zeros(Ncx+1, Ncy); kE[1:end-1,:] .= kc*0.0; kE[end,:] = kE[end-1,:]
    ηS = zeros(Ncx+1, Ncy); ηS            .= ηv[:,1:end-1]
    ηN = zeros(Ncx+1, Ncy); ηN            .= ηv[:,2:end-0]
    Cx = zeros(Ncx+1, Ncy);
    Cy = zeros(Ncx+1, Ncy);
    ...
    @.  λmaxloc = (Cx + Cy + 0*2/Δx)/Dx[:,2:end-1]
end
...
function GershgorinMechanics2Dy_Local!( λmaxloc, ηc, kc, ηv, Dy, Δx, Δy, Ncx, Ncy )
    ...
    @. λmaxloc .= (Cx + Cy + 0*2/Δy)/Dy[2:end-1,:]
end
...
function ResidualContinuity!(Rp, Vx, Vy, Pt, ηb, Dp, Δx, Δy)
...
@views function MainStokes2D()
    ...
    r          = nondimensionalize(0.1m, CharDim)
    ...
    for it=1:Nt
        ...
        λmaxPt = (2/Δx .+ 2/Δy .+ 1.0./ηb) ./Dp*0.4  
        ...
        @time @views for iter=1:niter
            ...
            ResidualContinuity!(Rp, Vx, Vy, Pt, ηb, Dp, Δx, Δy)
            ...
            @. ∂Pt∂τ                 = (2-c*hPt)/(2+c*hPt)*∂Pt∂τ + 2*hPt/(2+c*hPt).*Rp
            @. δPt                   = hPt*∂Pt∂τ 
            @. Pt                   += δPt
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                ResidualContinuity!(KδPt,  Vx.-δVx, Vy.-δVy, Pt.-δPt, ηb, Δx, Δy)

                λmin = abs(sum(.-δVx.*(KδVx1.-KδVx)) + sum(.-δVy.*(KδVy1.-KδVy)) + 0sum(.-δPt.*(KδPt1.-KδPt)) ) / (sum(δVx.*δVx) + sum(δVy.*δVy) + 0sum(δPt.*δPt))  
