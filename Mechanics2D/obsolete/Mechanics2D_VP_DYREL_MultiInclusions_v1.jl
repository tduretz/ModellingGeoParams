...
function ResidualContinuity!(Rp, Vx, Vy, Pt, ηb, ηc, Dp, Δx, Δy)
    ...
    @. Rp = -( (ηc./Δx) .* Pt./ηb + (Vx[2:end,2:end-1] - Vx[1:end-1,2:end-1])/Δx + (Vy[2:end-1,2:end-0] - Vy[2:end-1,1:end-1])/Δy )
    @. Rp /= Dp
...
@views function MainStokes2D()
    ...
    for it=1:Nt
        ...
        λmaxPt = (2/Δx .+ 2/Δy .+ ηc_maxloc/Δx./ηb) ./Dp   *15
        ...
        @time @views for iter=1:niter
            ...
            ResidualContinuity!(Rp, Vx, Vy, Pt, ηb, ηc, Dp, Δx, Δy)
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                ResidualContinuity!(KδPt1, Vx, Vy, Pt, ηb, ηc, Dp, Δx, Δy)
                ResidualContinuity!(KδPt,  Vx.-0*δVx, Vy.-0*δVy, Pt.-δPt, ηb, ηc, Dp, Δx, Δy)
