...
@views function MainMechanicsDiagPC2D(n, θ, dtP_fact, dtV_fact, θ_fact)
    ...
    for it=1:Nt
        ...
        dtPt .= 1.0/6.0*sqrt(min(Δx, Δy)) .- 0.0095
        dtPt .= 0.5*ηvec.* sqrt(min(Δx, Δy))
        @show ηvec[1]
...
function Optimize()
    ...
    for it=1:nt
        F = MainMechanicsDiagPC2D(1, θ, 1.08, 1.6, 0.76)
        
        # dFdθ 
        Fm   = MainMechanicsDiagPC2D(1, θ-5e-2*θ, 1.08, 1.6, 0.76)
        Fp   = MainMechanicsDiagPC2D(1, θ+5e-2*θ, 1.08, 1.6, 0.76)
        dFdθ = (Fp[1]-Fm[1]) /(10e-2*θ)
        θ-= F[1]/dFdθ

        θv[it] = θ 
        Fv[it] = Float64(F[1]) 

        display(plot(θv[1:it], Fv[1:it]))
        @show dFdθ
    end
end
