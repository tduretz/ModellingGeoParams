    ...
    for it=1:Nt
        ...
        @time @views for iter=1:niter
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                ResidualContinuity!(KδPt1, Vx,      Vy,      Pt,      bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, 1.0)
                ResidualContinuity!(KδPt,  Vx.-δVx, Vy.-δVy, Pt.-δPt, bp, ε̇, rheo, params, Dp, τ, τ0, Δx, Δy, Δt, 1.0, 1.0)
                λminPt = abs(sum(.-δPt.*(KδPt1.-KδPt))/(sum(δPt.*δPt) ) )
                @show (λminPt, hPt, cPt, (cPt/2)^2)
                @show 2/sqrt(λminPt)
                # cPt = 4*λminPt
                # @show hPt/cPt
