...
function main()
    ...
    #     probes.Vx0[it]  = 0.5*(Vx[end] + Vx[end-1])
        ...
        if mod(it, 10)==0 || it==1
            ...
            ax1 = Axis( f[1, 1], title = L"$$Fluid pressure", xlabel = L"$Pf$ [MPa]", ylabel = L"$y$ [km]" )
            lines!(ax1, ustrip.(dimensionalize(Pf, K, CharDim))/1e6, ustrip.(dimensionalize(yc, m, CharDim)./1e3) )
            lines!(ax1, ustrip.(dimensionalize(Pt, K, CharDim))/1e6, ustrip.(dimensionalize(yc, m, CharDim)./1e3) )
