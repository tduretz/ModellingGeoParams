...
function main()
    ...
    ϕ0      = 1e-3
    kf      = nondimensionalize(1e-10m^2, CharDim)
    ...
    sinψ    = sind(0.0)     # sinus of the dilation angle 
    ηvp     = nondimensionalize(0e19Pa*s, CharDim) # regularisation "viscosity"
    Ks      = nondimensionalize(18e10Pa, CharDim)                                 # solid compressibility
    Kf      = nondimensionalize(0.1e10Pa, CharDim)                                 # fluid compressibility
    ...
    Δt         = nondimensionalize(1e10s, CharDim)
    ...
    for it=1:Nt
        ...
        # # Update Δt
        # @show dimensionalize(Δt, s, CharDim)
        # dτIIdt = (mean(τII) - τII0)/Δt # Pa/s
        # Δt     = 0.005 * abs(mean(τII)) / abs(dτIIdt)
        @show  dimensionalize(Δt, s, CharDim), λ
        @. ηe       = G*Δt
        @. ηve      = 1.0/(1.0/ηv + 1.0/ηe)
        ...
        ΔτPf  = minimum(Δy^2/(kf/ηf)/2.1) /1e14
        ΔτPt  = ηv/Δy/1e5
        Δτϕ   = 2.1.*ηve./Ncy/500
        ...
        @views for iter=1:niter
            ...
            @. Rϕ           = - ( (ϕv - ϕv0)/Δt - ( ((Pf1 -Pf0)-(Pt1 -Pt0))/Δt/Kϕ + (Pf1 -Pt1 )/ηϕ + λ*sinψ) )
    ...
    #     probes.Vx0[it]  = 0.5*(Vx[end] + Vx[end-1])
        ...
        if mod(it, 50)==0 || it==1
            ...
            # Mohr circle
            peff  = LinRange(-100e6, 350e6, 100)
            τeff  = peff.*sinϕ .+ ustrip.(dimensionalize(τ_y,    Pa, CharDim)).*cosϕ
            θ     = LinRange(0, π, 100)
            _, imid = findmin(abs.(yv.-0.))
            τII_wall = ustrip.(dimensionalize(τII[1],    Pa, CharDim))./1e6
            Pt_wall  = ustrip.(dimensionalize(Pt[1],     Pa, CharDim))./1e6
            Pf_wall  = ustrip.(dimensionalize(Pf[1],     Pa, CharDim))./1e6
            τII_cent = ustrip.(dimensionalize(τII[imid], Pa, CharDim))./1e6
            Pt_cent  = ustrip.(dimensionalize(Pt[imid],  Pa, CharDim))./1e6
            Pf_cent  = ustrip.(dimensionalize(Pf[imid],  Pa, CharDim))./1e6
            ...
            p7 = plot(peff./1e6, τeff./1e6, xlabel="Peff", ylabel="τeff")
            p7 = plot!(τII_wall.*cos.(θ) .+ (Pt_wall-Pf_wall), τII_wall.*sin.(θ))
            p7 = plot!(τII_cent.*cos.(θ) .+ (Pt_cent-Pf_cent), τII_cent.*sin.(θ))
