...
function main()
    ...
    sinψ    = sind(10)     # sinus of the dilation angle 
    ηvp     = nondimensionalize(5e18Pa*s, CharDim) # regularisation "viscosity"
    ...
    Δt         = nondimensionalize(0.5e10s, CharDim)

    # Allocate arrays
    Pt         = zeros(Ncy+1)
    Pt        .= -0*ΔPf .*exp.(-yv.^2/2σ^2) 
    Pf         = zeros(Ncy+1) 
    Pf        .= 0*ΔPf .*exp.(-yv.^2/2σ^2)
    Pt0        = zeros(Ncy+1) 
    Pf0        = copy(Pf)
    Rϕ         =   zeros(Ncy+1)
    ϕv         = ϕ0*ones(Ncy+1) 
    ...
    σxxB       = nondimensionalize(-8e6Pa, CharDim) # Courbe B - Vermeer
    σyyB       = nondimensionalize(-2e6Pa, CharDim) # Courbe B - Vermeer
    σzzB       = σxxB
    ...
    for it=1:Nt
        ...
        @views for iter=1:niter
            ...
            @. λ = (F>0) * F/( (1.0-ϕv)*ηve + ηvp)# .+ (mt .- yesPf*mf)*sinϕ*sinψ)
            ...
            @. τII     =  sqrt( 0.5*(τxx^2 + τyy^2 + τzz^2) + τxy^2)
            @. F =  τII - (Pt-Pf)*sinϕ - τ_y*cosϕ - λ*ηvp
            ...
            @. Rϕ           = - ( (ϕv - ϕv0)/Δt - ((Pf -Pf0)-(Pt -Pf0))/Δt/Kϕ + (Pf -Pt )/ηϕ )
