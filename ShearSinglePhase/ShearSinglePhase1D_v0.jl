...
function main()
    ...
    τxx        = τxxi*ones(Ncy+1)
    τxy        = τxyi*ones(Ncy+1)
    τyy        = τyyi*ones(Ncy+1)  #.+ Δτ.*exp.(-yv.^2/2σ^2)  
    τzz        = τzzi*ones(Ncy+1)  #.+ Δτ.*exp.(-yv.^2/2σ^2)  
    τxx0       = τxxi*ones(Ncy+1)
    τxy0       = τxyi*ones(Ncy+1)
    τyy0       = τyyi*ones(Ncy+1)  #.+ Δτ.*exp.(-yv.^2/2σ^2) 
    τzz0       = τzzi*ones(Ncy+1)  #.+ Δτ.*exp.(-yv.^2/2σ^2) 
    Coh        = zeros((Ncy+1)); Coh .= Coh0 .- Coh0./10 .* exp.(-yv.^2/2σ^2)
    F          = zeros((Ncy+1))
    Fc         = zeros((Ncy+1))
    λ̇          = zeros((Ncy+1))
    λ̇rel       = zeros((Ncy+1))
    ...
    for it=1:Nt
        ...
        @views for iter=1:niter
            ...
            elseif BC_Vy == :Neumann   
                Vy[end] =  Vy[end-1] + Δy/(4//3*ηve[end] + Kb*Δt)*(σyyi + Pt0[end] - τyy0[end]/ηe*ηve[end])
            end
            ...
            for pl in eachindex(F)
                ...
                if F[pl] > 0.
                    ispl[pl] = 1
                    λ̇[pl]    = F[pl] / (ηvp + ηve[pl] + Kb*Δt*sin(ϕ)*sin(ψ))
        ...
        if mod(it, 10)==0 || it==1
            ...
            p3=plot(title = "σ1 angle", xlabel = L"$angle$ [kPa]", ylabel = L"$y$ [km]" )
            p3=plot!(ustrip.(-atand.(σ1.z[:] ./ σ1.x[:])), ustrip.(dimensionalize(yv, m, CharDim)./1e3), xlimits=(10,60) )

            p4=plot(title = "Probes", xlabel = "Angle", ylabel = L"[-]" )
