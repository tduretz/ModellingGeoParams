...
@inline function Residuals!(R, τ, τ0, Pt, Pt0, ε̇, ∇v, V, BC, arrays, yield, rel, NL, Δt, Δy)
    # Kinematics
    # Vx BC
...
@inline @views function GershgorinMomentumX(η, Δy)
    return maximum(2*(η[1:end-1] .+ η[2:end])./Δy^2)
end
...
function main()
    ...
    arrays.ηe    .= G*Δt
    arrays.Kb[50] = 4 *Kbulk #.- Coh0 .* exp.(-yv.^2/2σ^2)
    ...
    ∂Vx∂τ      =    zeros(Ncy+2)
    ∂Vy∂τ      =    zeros(Ncy+2)
    σ1         = (x=zeros(size(τ.xx)), z=zeros(size(τ.xx)) )

    # Monitoring
    probes    = (ẆB =zeros(Nt), τxyB=zeros(Nt), σyyB=zeros(Nt), VxB=zeros(Nt), τII=zeros(Nt))
    η        .= μs
    ...
    # δV    = (x = zeros(Ncy+2), y = zeros(Ncy+2))
    iters = 0
    ...
    for it=1:Nt
        ...
        λmin   = 1.0
        c      = 2*sqrt(λmin)
        λmaxVx = GershgorinMomentumX(arrays.ηve, Δy)
        λmaxVy = GershgorinMomentumY(arrays.ηve, arrays.Kb, Δt, Δy)
        ...
        for iter=1:niter
            ...
            Residuals!(R, τ, τ0, Pt, Pt0, ε̇, ∇v, V, BC, arrays, yield, rel, true, Δt, Δy)
            ...
            # PT time steps
            @. η_maxloc  = max.(arrays.ηvep[1:end-1], arrays.ηvep[2:end]); 
            
            # DYREL
            @. ∂Vx∂τ         = (2-c*hVx)/(2+c*hVx)*∂Vx∂τ + 2*hVx/(2+c*hVx)*R.x
            @. V.x[2:end-1] += hVx*∂Vx∂τ[2:end-1]
            
            @. ∂Vy∂τ         = (2-c*hVy)/(2+c*hVy)*∂Vy∂τ + 2*hVy/(2+c*hVy)*R.y
            @. V.y[2:end-1] += hVy*∂Vy∂τ[2:end-1]
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                Residuals!(KδV,   τ, τ0, Pt, Pt0, ε̇, ∇v, V,   BC, arrays, yield, rel, true, Δt, Δy)
                Residuals!(KδVit, τ, τ0, Pt, Pt0, ε̇, ∇v, Vit, BC, arrays, yield, rel, true, Δt, Δy)
                ...
                Residuals!(R, τ, τ0, Pt, Pt0, ε̇, ∇v, V, BC, arrays, yield, rel, true, Δt, Δy)
                errVx = norm(R.x)/sqrt(length(R.x))
                errVy = norm(R.y)/sqrt(length(R.y))
        ...
        if mod(it, 10)==0 || it==1 
            ...
            app_fric =  ustrip.(-probes.τxyB[1:it]./probes.σyyB[1:it])/2
