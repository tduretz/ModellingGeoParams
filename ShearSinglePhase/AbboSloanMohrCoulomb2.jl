...
function Fλ(λ̇, τ, σm, ϕ, ψ, c, θt, G, K, Δt, τxx0, τyy0, τzz0, τxy0, P0, ε̇xx, ε̇yy, ε̇zz, ε̇xy, ∇v)
    𝐹τ = τ -> Yield_MCAS95(τ, σm, ϕ, c, θt )
    ∂Q∂τ = ForwardDiff.gradient( 𝐹τ, τ)
...
function main_MCAS95()
    ...
    for it=1:nt
        ...
        if F>0
            ...
            for _=1:200
                iter +=1
                F     = Fλ(λ̇, [τxx; τyy; τzz; τxy; τxz; τyz], σm, ϕ, ψ, c, θt, G, K, Δt, τxx0, τyy0, τzz0, τxy0, P0, ε̇xx, ε̇yy, ε̇zz, ε̇xy, ∇v)
                iter==1 ? F0 = F :  nothing
                @show F/F0
                abs(F/F0) < 1e-10 ? break : nothing
                𝐹𝜆    = λ̇ -> Fλ(λ̇, [τxx; τyy; τzz; τxy; τxz; τyz], σm, ϕ, ψ, c, θt, G, K, Δt, τxx0, τyy0, τzz0, τxy0, P0, ε̇xx, ε̇yy, ε̇zz, ε̇xy, ∇v)
                ∂F∂λ̇  = ForwardDiff.derivative(𝐹𝜆, λ̇)
                Δλ̇    = F/∂F∂λ̇
                Fmin .= 𝐹𝜆.(λ̇ .- α*Δλ̇)
                _,ii  = findmin(abs.(Fmin))
                λ̇    -= α[ii]*Δλ̇
                𝐹τ     = τ -> Yield_MCAS95(τ, σm, ϕ, c, θt )
                # ∂Q∂τ = ForwardDiff.gradient( 𝐹τ, [τxx; τyy; τzz; τxy; τxz; τyz])
                # τxx  = τxx0 + 2*G*Δt*(ε̇xx - λ̇*∂Q∂τ[1])
                # τyy  = τyy0 + 2*G*Δt*(ε̇yy - λ̇*∂Q∂τ[2])
                # τzz  = τzz0 + 2*G*Δt*(ε̇zz - λ̇*∂Q∂τ[3])
                # τxy  = τxy0 + 2*G*Δt*(ε̇xy - λ̇*∂Q∂τ[4])
