...
@inline @views function GershgorinMomentumY(η, Kb, Δt, Δy)
    return maximum(0.333333*(η[1:end-1] .+ η[2:end])./Δy^2 .+ 0.333333*η[2:end]./Δy^2 .+ 0.333333*η[1:end-1]./Δy^2 .+ 0*2/Δy .+ 2.0*Kb[1:end-1].*Δt./Δy^2 .+ 2.0*Kb[2:end-0].*Δt./Δy^2)
end

@inline @views function GershgorinContinuity(Kb, Δt, Δy)
    return maximum(ones(size(Kb)))
end
...
function main()
    ...
    for it=1:Nt
        ...
        λminVy = 1.0
        λmaxVy = GershgorinMomentumY(ηve, Kb, Δt, Δy)*GershVy
        hVy = 2/sqrt(λmaxVy)*CFL
        cVy = 2*sqrt(λminVy)

        λminPt = 1.0
        λmaxPt =  0.8e7#GershgorinContinuity(Kb, Δt, Δy)*GershPt
        hPt = 2/sqrt(λmaxPt)*CFL
        cPt = 2*sqrt(λminPt)

        hPt = (1.0./(2.0./sqrt.(Kb[1].*Δt)))*CFL
        ...
        @views for iter=1:niter
            ...
            # @. ∂Pt∂τ        = (2.0-cPt*hPt)/(2.0+cPt*hPt)*∂Pt∂τ + 2.0*hPt/(2.0+cPt*hPt)*RPt
            # @. δPt          = hPt*∂Pt∂τ
            # @. Pt          += δPt

            # Kinematics!(ε̇xy, ε̇yy, bV, Vy, Δy)
            # Rheology!(τxx, τyy, τzz, τxy, Ptc,  τii, F,  Fc, ε̇yy, ε̇xy, Pt, τxx0, τyy0, τzz0, τxy0, Kb, Δt, ηe, ηve, ηvep, Coh, ϕ, ψ, ηvp, λ̇, λ̇rel, ε̇iiᵉᶠᶠ, ispl, rel )
            ...
            # Kinematics!(ε̇xy, ε̇yy, Vx, Vy, Δy)
            # Rheology!(τxx, τyy, τzz, τxy, bP,  τii, F,  Fc, ε̇yy, ε̇xy, bP, τxx0, τyy0, τzz0, τxy0, Kb, Δt, ηe, ηve, ηvep, Coh, ϕ, ψ, ηvp, λ̇, λ̇rel, ε̇iiᵉᶠᶠ, ispl, rel )
            # ResidualContinuity!(KδP1, bP, Pt0, Vy, Δy, Kb, Δt)
            # bP .= KδP1 ./ norm(KδP1)
            # @show bP'*KδP1 / (bP'*bP)
            ...
            if mod(iter, nout) == 0 || iter==1
                ...
                # @show (ηve[1], minimum(Kb), maximum(Kb), Δt, Δy)

                # @show λmaxPt = GershgorinContinuity(Kb, Δt, Δy)*GershPt
                # Kinematics!(ε̇xy, ε̇yy, Vx, Vy, Δy)
                # Rheology!(τxx, τyy, τzz, τxy, Ptc,  τii, F,  Fc, ε̇yy, ε̇xy, Pt, τxx0, τyy0, τzz0, τxy0, Kb, Δt, ηe, ηve, ηvep, Coh, ϕ, ψ, ηvp, λ̇, λ̇rel, ε̇iiᵉᶠᶠ, ispl, rel )
                # ResidualContinuity!(KδP1, Pt, Pt0, Vy, Δy, Kb, Δt)
                # Kinematics!(ε̇xy, ε̇yy, Vx, Vy, Δy)
                # Rheology!(τxx, τyy, τzz, τxy, Ptc,  τii, F,  Fc, ε̇yy, ε̇xy, Pt.-δPt, τxx0, τyy0, τzz0, τxy0, Kb, Δt, ηe, ηve, ηvep, Coh, ϕ, ψ, ηvp, λ̇, λ̇rel, ε̇iiᵉᶠᶠ, ispl, rel )
                # ResidualContinuity!(KδP, Pt.-δPt, Pt0, Vy, Δy, Kb, Δt)
                # @show λminPt =  abs(sum(.-δPt.*(KδP1.-KδP))/sum(δPt.*δPt) / 1.0 )
                # hPt = 2/sqrt(λmaxPt)*CFL
                # cPt = 2*sqrt(λminPt)
                ...
                # @show ΔτPt[1]
                # @show hPt
                # @show (2-cPt*hPt)/(2+cPt*hPt)
                # @show 2*hPt/(2+cPt*hPt)

                @show maximum(1.0./(Kb*Δt)/ηve[1]/Δy^2) 
                @show minimum(1.0./(Kb*Δt)/ηve[1]/Δy^2)

                # @show maximum(Kb*Δt/ηve[1]) 
                # @show minimum(Kb*Δt/ηve[1])

                # @show maximum(Kb*Δt/ηve[1]/Δy) 
                # @show minimum(Kb*Δt/ηve[1]/Δy)
