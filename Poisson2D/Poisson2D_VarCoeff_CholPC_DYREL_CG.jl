...
@views function MainPoisson2D(n, auto, solver, PC, nout)
    ...
    for it=1:Nt
        ...
        @views for iter=1:niter
            ...
            # Pseudo-old preconditioned residuals 
            RT_PC0 .= RT_PC
            ...
            # Updates
            @. ∂T∂τ                  = (2-cT*hT)/(2+cT*hT)*∂T∂τ + 2*hT/(2+cT*hT)*RT_PC
            @. δT                    = hT*∂T∂τ
            @. Tc[2:end-1,2:end-1] .+= δT[2:end-1,2:end-1]

            # Checks with nout trigger
            if mod(iter, nout) == 0 || iter==1
                λminT =  abs(sum(.-δT.*(RT_PC .- RT_PC0))/sum(δT.*δT) )
                ...
                @printf("fT = %2.4e\n", errT)
                mod(iter, nout) == 0 ? err[iter-nout+1:iter] .= errT : err[iter] = errT
        ...
        elseif solver==:PCG
            ...
            @views for iter=1:niter
                ...
                # Ap
                ResidualThermics2D!(Ap, p, Tc0, Qr, Δt, ρ, Cp, k, Δx, Δy, ∂T∂x, ∂T∂y, qTx, qTy, 0.0, T_South, T_North, DM, false )
                Ap *= -1.
                
                # α
                dot0         = dot(RT[2:end-1,2:end-1], RT_PC[2:end-1,2:end-1])
                α_CG         = dot0 / dot(p[2:end-1,2:end-1], Ap[2:end-1,2:end-1]) 
                
                # Updates 1
                Tc[2:end-1,2:end-1] .+= α_CG .*  p[2:end-1,2:end-1] 
                RT[2:end-1,2:end-1] .-= α_CG .* Ap[2:end-1,2:end-1]
                ...
                # Update 2
                p[2:end-1,2:end-1]  .= RT_PC[2:end-1,2:end-1] .+ β_CG.*p[2:end-1,2:end-1]
...
@info "DYREL"
MainPoisson2D(4, true, :DYREL, :diag,  100)
MainPoisson2D(4, true, :DYREL, :ilu,   100)
MainPoisson2D(4, true, :DYREL, :ichol, 100)

@info "PCG"
MainPoisson2D(4, true, :PCG, :diag,  100)
MainPoisson2D(4, true, :PCG, :ilu,   100)
MainPoisson2D(4, true, :PCG, :ichol, 100)